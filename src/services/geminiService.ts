...
export const analyzeDocumentation = async (docs: { name: string; content: string }[]): Promise<SystemModel> => {
  ...
  const model: SystemModel = JSON.parse(response.text || "{}");
  model.smartSuggestions = evaluateRules(model);
  return model;
};

export const quickAnalyze = async (docs: { name: string; content: string }[]): Promise<SystemModel> => {
  const combinedDocs = docs.map(d => `File: ${d.name}\n\n${d.content}`).join("\n\n---\n\n");
